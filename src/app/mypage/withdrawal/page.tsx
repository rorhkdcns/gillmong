  ...
  return (
    <>
      ...
      <div className="mx-auto w-full max-w-2xl flex-1 px-4 py-8 md:px-6 md:py-12">
        ...
        {/* 출금 신청 폼 */}
        <section className="mb-8 border border-gray-200 bg-white p-5 md:p-8">
          <h2 className="mb-5 bg-[#01273A] px-3 py-1 text-sm font-semibold text-white inline-block">신청 정보</h2>
          {balance < 5000 ? (
            <div className="rounded border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-700">
              출금 가능한 포인트가 부족합니다. (최소 5,000P)
            </div>
          ) : (
            <WithdrawalForm balance={balance} realName={realName} />
          )}
        </section>
        ...
        {(history ?? []).length > 0 && (
          <section className="border border-gray-200 bg-white p-5 md:p-8">
            <h2 className="mb-4 bg-[#01273A] px-3 py-1 text-sm font-semibold text-white inline-block">신청 내역</h2>
                  ...
                  <li key={row.id} className="flex items-center justify-between py-3">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-[#999]">
                        {row.bank_name} · {row.account_number} · {date}
                      </p>
                    </div>
