...
import { runShopProductGeneration } from '@/lib/shopProductGeneration'
...
// 관리자 화면의 카테고리/하위카테고리 행 버튼으로 대체한다. 핵심 로직은
// /api/cron/shop-collect(매일 자동 수집)와 공유하기 위해 lib/shopProductGeneration.ts로
// 분리되어 있고, 이 서버 액션은 admin 인증만 담당한다.
export async function generateShopProducts(
  ...
  return runShopProductGeneration(categoryId, subcategoryId)
}
